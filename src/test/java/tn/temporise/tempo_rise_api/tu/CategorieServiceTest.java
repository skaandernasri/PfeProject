...
public class CategorieServiceTest {
    ...
    void testCreateCategorie() {
        ...
        when(categorieMapper.dtoToModel(categorieRequest)).thenReturn(categorie); // Map request to model
        when(categorieRepo.save(categorie)).thenReturn(categorie); // Simulate saving the category
        when(categorieMapper.modelToResponse(categorie)).thenReturn(categorieResponse); // Map model to response
        ...
        assertEquals(1L, response.getId(), "The ID should match");
        assertEquals("Electronics", response.getNom(), "The name should match");
        verify(categorieRepo, times(1)).save(categorie); // Verify that save was called
    }
    ...
    void testUpdateCategorie() {
        // Mock des dépendances
        when(categorieRepo.findById(1L)).thenReturn(categorie); // Simulate fetching the existing category
        when(categorieRepo.update(categorie)).thenReturn(categorie); // Simulate updating the category
        when(categorieMapper.modelToResponse(categorie)).thenReturn(categorieResponse); // Map model to response
